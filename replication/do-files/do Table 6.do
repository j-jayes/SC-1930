...
** s_ag
* the share (in percent) of the population in agriculture:
...
* (1) Log(1+Labor force)	
xtreg llabforce i.year iline iline_owl i.irail L10.llabforce L10.shc2-shc7 i.county_id#i.year, fe vce(cluster parish_code)
est sto llabforce

* (2) Share of population in agriculture (%)
xtreg s_ag i.year iline iline_owl i.irail L10.llabforce L10.shc2-shc7 i.county_id#i.year, fe vce(cluster parish_code)
est sto s_ag


* create regression table with results
esttab llabforce s_ag, drop(*.year *year _cons) s(N , labels("Observations")) ///
mlabels("Log(1+Labor force)" "Share of population in agriculture (%)") se
