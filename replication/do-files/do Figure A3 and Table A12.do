...
use "data/data Figure A3 and Table A12", clear
...
** iurb
* dummy variable denoting if parish was among the 50 percent of parishes with the highest population density in 1900
...
/////////////////////////////////////////
/////// Figure A3 and Table A12: Results for Strikes by Type of Parish: Rural/Urban

** run regressions and save results for esttab table 

* (1) Log(1+Total Strikes)	
xtreg lstrikes i.year iline#iurban iline_owl i.irail L10.llabforce L10.shc2-shc7 i.county_id#i.year, fe vce(cluster parish_code)
est sto lstrikes
* calculate margins for figure
margins, dydx(1.iline) at(iurban=(0 1) iline=0) noestimcheck

* (2) Log(1+Offensive Strikes)	
xtreg lstrikesoff i.year iline#iurban iline_owl i.irail L10.llabforce L10.shc2-shc7 i.county_id#i.year, fe vce(cluster parish_code)
est sto lstrikesoff
* calculate margins for figure
margins, dydx(1.iline) at(iurban=(0 1) iline=0) noestimcheck

* (3) Log(1+Defensive Strikes)	
xtreg lstrikesdef i.year iline#iurban iline_owl i.irail L10.llabforce L10.shc2-shc7 i.county_id#i.year, fe vce(cluster parish_code)
est sto lstrikesdef
* calculate margins for figure
margins, dydx(1.iline) at(iurban=(0 1) iline=0) noestimcheck
...
** graph regression results (figure A3)
...
gen id=_n
		...
		// strikes
gen b=10.39677 if id==1
gen cil=2.890299 if id==1
gen cih=17.90324 if id==1
replace b=13.05834 if id==2
replace cil=-1.724178 if id==2
replace cih=27.84086 if id==2
		// strikeswage
replace b=9.66941 if id==4
replace cil=1.938367 if id==4
replace cih=17.40045 if id==4
replace b=10.7227 if id==5
replace cil=-2.620842 if id==5
replace cih=24.06625 if id==5
		// strikesdef
replace b=3.358995 if id==7
replace cil=.023949 if id==7
replace cih=6.694042 if id==7
replace b=3.649499 if id==8
replace cil=-1.157137 if id==8
replace cih=8.456134 if id==8
...
ylabel(0 "{bf:Strikes}" 1 "Rural" 2 "Urban" 3 "{bf:Offensive strikes}" 4 "Rural" 5 "Urban" 6 "{bf:Defensive strikes}" 7 "Rural" 8 "Urban", angle(0) labsize(medium)) yscale(reverse) ytitle("") xtitle("Change (%)", size(medium)) ///
